...
class Items extends BaseController
{
  ...
  {
    $countries = $this->locationModel->where('parent_id', null)
      ->orderBy('en_name', 'ASC')
      ->findAll();

    $cities = [];

    $sub_categories = $this->categoryModel->where('parent_id !=', null)
      ->orderBy('en_name', 'ASC')
      ->findAll();

    $data['sources'] = $this->sourceModel->findAll();
    $data['categories'] = $this->categoryModel->where('parent_id', null)->findAll(); // chỉ category cha
    $data['sub_categories'] = $sub_categories; // sub categories
  ...
  public function edit($id)
  {
    ...
    $cities = $this->locationModel->where('parent_id', $item['country_id'])
      ->orderBy('en_name', 'ASC')
      ->findAll();
